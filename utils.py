...
def check_accuracy(loader, model, device="cuda"):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            dice_score += (2 * (preds * y).sum()) / (  # Summing the number of pixels where they are both the same (i.e. outputting a white pixel)
                (preds + y).sum() + 1e-8  # Summing the number of white pixels for preds and label
            )
